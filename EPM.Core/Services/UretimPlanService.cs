...
{
    public class UretimPlanService : IUretimPlanService
    {
        public object GetPlan(HttpContext context,int BRAND, int SEASON, string MODEL, string COLOR, int ORDER_TYPE, int PRODUCTION_TYPE, int FABRIC_TYPE)
        {
            ...
            sql += @"GROUP BY H.ID,
                             ...
                             MR.ADI, MR.ID
                    ORDER BY H.ID) A";
            DataTable dt = OracleServer.QueryFill(sql);
            string tSql = "SELECT ID FROM ( " + sql + " )B";
            EPM_PRODUCTION_SEASON_WEEKS seasonWeek = OracleServer.Deserialize<EPM_PRODUCTION_SEASON_WEEKS>("SELECT * FROM FDEIT005.EPM_PRODUCTION_SEASON_WEEKS WHERE SEASON_ID=" + SEASON + "");
            ...
            List<EPM_PRODUCTION_PLAN> plan = OracleServer.DeserializeList<EPM_PRODUCTION_PLAN>(@"SELECT * FROM FDEIT005.EPM_PRODUCTION_PLAN PL
        ...
        public object UpdateInsertPlan(HttpContext context,JObject obj)
        {
            ...
            if (newItem.Count > 0)
            {
                ...
                int tumPlanlananlar = OracleServer.ReadInteger(string.Format(@"
                ...
                if (URETIMADET - tumPlanlananlar >= QUANTITY)
                {
                    ...
                    EPM_PRODUCTION_PLAN plan = OracleServer.Deserialize<EPM_PRODUCTION_PLAN>(sql);
                    plan.MARKET_ID = MARKET_ID;
                    plan.HEADER_ID = HEADER_ID;
                    plan.WEEK = WEEK;
                    plan.YEAR = YEAR;
                    plan.QUANTITY = QUANTITY;
                    plan.CREATED_BY = login.USER_CODE;
                    OracleServer.Serialize(plan);
                    OracleServer.ExecSql("DELETE FDEIT005.EPM_PRODUCTION_PLAN WHERE QUANTITY=0");
                }
                else
                {
                    response.isOK = false;
        ...
        {
            IEnumerable< EPM_PRODUCTION_BAND_CAPASITIES> list= OracleServer.DeserializeList<EPM_PRODUCTION_BAND_CAPASITIES>("SELECT * FROM FDEIT005.EPM_PRODUCTION_BAND_CAPASITIES");
            return list;
        }
        public IEnumerable<KapasitePlanListChart> GetKapasitePlanListChart()
        {
            IEnumerable<KapasitePlanListChart> list = OracleServer.DeserializeList<KapasitePlanListChart>(@"SELECT BG.ADI BAND_NAME,CAPACITY FROM FDEIT005.EPM_PRODUCTION_BAND_CAPASITIES CP
INNER JOIN FDEIT005.EPM_PRODUCTION_BAND_GROUP BG ON BG.ID = CP.BAND_ID");
        ...
        public object[] KapasitePlanListUpdate(int Key, string Values)
        {
            ...
            try
            {
                EPM_PRODUCTION_BAND_CAPASITIES detail = OracleServer.Deserialize<EPM_PRODUCTION_BAND_CAPASITIES>(@"SELECT * FROM FDEIT005.EPM_PRODUCTION_BAND_CAPASITIES D  WHERE D.ID=" + Key);
                JsonConvert.PopulateObject(Values, detail);
                OracleServer.Serialize<EPM_PRODUCTION_BAND_CAPASITIES>(detail);
            }
            catch (Exception ex)
            {
                ok[0] = false;
...
SELECT CP.BAND_ID, BG.ADI AS BAND_NAME, CP.CAPACITY,A.WEEK
    FROM FDEIT005.EPM_PRODUCTION_BAND_CAPASITIES CP
         ...
         UNION ALL
         SELECT 53 WEEK FROM DUAL
         UNION ALL
         SELECT 54 WEEK FROM DUAL
         ) A
...
WHERE BAND_ID={1} ", YEAR, BAND_GROUP);
            IEnumerable<KapasitePlanUyum> kapasite = OracleServer.DeserializeList<KapasitePlanUyum>(sql);
