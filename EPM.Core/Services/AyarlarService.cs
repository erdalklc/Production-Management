            ...
            using (var principalContext = new PrincipalContext(ContextType.Domain, "eren.holding"))
            {
                using (var searcher = new PrincipalSearcher(new UserPrincipal(principalContext)))
                {
                    ...
                    users.ForEach(us =>
                    {
                        ...
                        l.USER_CODE = us.Name;
                        l.USER_MAIL = us.EmailAddress;
                        l.DESCIRIPTION = us.DisplayName;
                        liste.Add(l);
                    });
                }
            }
...
", USER_CODE);
            List<KullaniciYetki> res = OracleServer.DeserializeList<KullaniciYetki>(sql);
            return res;
        }
...
ORDER BY FABRIC_TYPE_ID", USER_CODE);
            ...
            List<KullaniciFabricTypes> res = OracleServer.DeserializeList<KullaniciFabricTypes>(sql);
            return res;
        }
...
ORDER BY BRAND_ID", USER_CODE);
            ...
            List<KullaniciMarka> res = OracleServer.DeserializeList<KullaniciMarka>(sql);
            return res;
        }
...
WHERE USER_CODE='{0}' ) A )B ON B.PRODUCTION_TYPE_ID = R.ID", USER_CODE);
            ...
            List<KullaniciProductionTypes> res = OracleServer.DeserializeList<KullaniciProductionTypes>(sql);
            return res;
        }
        ...
        public object[] UserBrandsUpdate(int key, string values, string USER_CODE)
        {
            ...
            try
            {
                ...
                yetki.BRAND_ID = key;
                EPM_USER_BRANDS res = OracleServer.Deserialize<EPM_USER_BRANDS>(String.Format("SELECT * FROM FDEIT005.EPM_USER_BRANDS WHERE USER_CODE='{0}' AND BRAND_ID={1}", yetki.USER_CODE, yetki.BRAND_ID));
                ...
                if (yetki.ISACTIVE)
                {
                    ...
                    if (res.ID.IntParse() == 0)
                    {
                        ...
                        OracleServer.Serialize(res);
                    }
                }
                else
                {
                    OracleServer.ExecSql(String.Format("DELETE FROM FDEIT005.EPM_USER_BRANDS WHERE USER_CODE='{0}' AND BRAND_ID={1}", yetki.USER_CODE, yetki.BRAND_ID));
                }
            }
            catch (Exception ex)
            {
        ...
        public object[] UserResponsibilityUpdate(int key, string values, string USER_CODE)
        {
            ...
            try
            {
                ...
                EPM_USER_RESPONSIBILITY res = OracleServer.Deserialize<EPM_USER_RESPONSIBILITY>(String.Format("SELECT * FROM FDEIT005.EPM_USER_RESPONSIBILITY WHERE USER_CODE='{0}' AND RESPONSIBILITY_ID={1}", yetki.USER_CODE, yetki.RESPONSIBILITY_ID));
                ...
                {
                    OracleServer.ExecSql(String.Format("DELETE FROM FDEIT005.EPM_USER_RESPONSIBILITY WHERE USER_CODE='{0}' AND RESPONSIBILITY_ID={1}", yetki.USER_CODE, yetki.RESPONSIBILITY_ID));
                }
            }
            catch (Exception ex)
            {
        ...
        public object[] UserFabricTypeUpdate(int key, string values, string USER_CODE)
        {
            ...
            try
            {
                ...
                EPM_USER_FABRIC_TYPES res = OracleServer.Deserialize<EPM_USER_FABRIC_TYPES>(String.Format("SELECT * FROM FDEIT005.EPM_USER_FABRIC_TYPES WHERE USER_CODE='{0}' AND FABRIC_TYPE_ID={1}", yetki.USER_CODE, yetki.FABRIC_TYPE_ID));
                ...
                {
                    OracleServer.ExecSql(String.Format("DELETE FROM FDEIT005.EPM_USER_FABRIC_TYPES WHERE USER_CODE='{0}' AND FABRIC_TYPE_ID={1}", yetki.USER_CODE, yetki.FABRIC_TYPE_ID));
                }
            }
            catch (Exception ex)
            {
        ...
        public object[] UserProductionTypeUpdate(int key, string values, string USER_ID)
        {
            ...
            try
            {
                ...
                EPM_USER_PRODUCTION_TYPES res = OracleServer.Deserialize<EPM_USER_PRODUCTION_TYPES>(String.Format("SELECT * FROM FDEIT005.EPM_USER_PRODUCTION_TYPES WHERE USER_CODE='{0}' AND PRODUCTION_TYPE_ID={1}", yetki.USER_CODE, yetki.PRODUCTION_TYPE_ID));
                ...
                {
                    OracleServer.ExecSql(String.Format("DELETE FROM FDEIT005.EPM_USER_PRODUCTION_TYPES WHERE USER_CODE='{0}' AND PRODUCTION_TYPE_ID={1}", yetki.USER_CODE, yetki.PRODUCTION_TYPE_ID));
                }
            }
            catch (Exception ex)
            {
