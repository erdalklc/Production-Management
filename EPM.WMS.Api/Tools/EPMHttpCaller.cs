...
namespace EPM.WMS.Api.Tools
{
    public abstract class EPMHttpCaller
    {
        ...
        protected static LoginReturnModel GetLogin(EPMServiceType serviceType,string apiUrl, string EmailAddress, string Pwd)
        {
            var rval = default(LoginReturnModel);
            using (EPMHttpClient client = new EPMHttpClient(serviceType))
            {
                HttpResponseMessage message = client.GetLogin(apiUrl, EmailAddress, Pwd).Result;
                if(message.IsSuccessStatusCode)
                {
                    ...
                    try
                    {
                        ...
                        rval = JsonConvert.DeserializeObject<LoginReturnModel>(data);
                    }
                    catch (Exception)
                    {

                        LoadFailureEvent(message, true);
                    }
                }
            }
            return rval;
