...
namespace EPM.Tools.Managers
{
    public class AppFilterAttribute : ActionFilterAttribute
    {
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            var user = new CookieHelper().GetObjectFromCookie<WebLogin>(filterContext.HttpContext,"USER");
            if (actionName == "Login" || actionName == "LoginKontrol")
            {
                if (actionName == "LogOut" || actionName == "Login")
                    new CookieHelper().DeleteCookie(filterContext.HttpContext,"USER");
            }
            else
            { 
